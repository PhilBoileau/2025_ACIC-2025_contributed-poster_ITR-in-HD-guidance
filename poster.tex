...
\begin{columns}
  ...
  \myblock{Problem Formulation}{
    ...
    random vector of pre-treatment covariates (and possible confounders), $A$ is
    a binary treatment indicator, and $Y$ is the continuous outcome. Here, $n<p$
    or $n \approx p$.
  ...
  \myblock{Primary Objectives}{
    \begin{itemize}
        \item \textbf{Provide practical guidance to applied scientists on ITR
          estimation in high-dimensional settings, and }
        \item \textbf{Assess the impact of treatment effect modifier filtering
          on ITR estimaton}
    \end{itemize}
  }
  ...
  \myblock{Estimators}{
    ...
    \citet{boileauNonparametricFrameworkTreatment2025} were used to define ITR
    estimators as well.
